...
# Resolve @worktree, @worktree/, @worktree/s, @worktree/c, @worktree/ss, or @worktree/cc
...
function Resolve-WorktreePath {
    ...
    # Match @worktree/ or @worktree (base folder)
    elseif ($Path -match '^@([^/]+)/?$') {
        $wt = $Matches[1]
        $match = Get-Worktrees | Where-Object { $_ -like "*$wt*" } | Select-Object -First 1
        if ($match) {
...
# Register completer for cd, sl, Set-Location
Register-ArgumentCompleter -CommandName @('Set-Location', 'cd', 'sl') -ParameterName Path -ScriptBlock {
    param($cmd, $param, $word)

    if ($word -match '^@(.*)') {
        # List worktrees with trailing /
        $filter = $Matches[1] -replace '/$', ''
...
Set-PSReadLineKeyHandler -Key Enter -ScriptBlock {
    ...
    # Replace @worktree, @worktree/, @worktree/s, @worktree/c, @worktree/ss, or @worktree/cc
    if ($line -match '^(cd|Set-Location|sl)\s+(@[^/\s]+/?[sc]{0,2})(.*)$') {
        $cmd = $Matches[1]
        $wtPath = $Matches[2]
        $rest = $Matches[3]
