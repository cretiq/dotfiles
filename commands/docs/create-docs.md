...
Search **both** skill locations before creating anything:

**Project skills:**
```
Glob: ~/.dotfiles/skills/*/SKILL.md
```
...
2. **Map file paths** — document real locations
...
**Location decision:**
- **Default:** `~/.dotfiles/skills/{name}/` — dotfiles-specific knowledge
- **Global only if** the skill clearly applies beyond this project (e.g., generic tooling, cross-project patterns) — use `~/.claude/skills/{name}/`
...
Include all of:
- What it does
- Specific tech with versions
- Tool integrations if applicable (Context7, Chrome DevTools MCP)
...
- **Default to project scope** — only go global if genuinely cross-project
