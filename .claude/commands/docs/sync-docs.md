...
3. Group changed files into feature domains (e.g., "ghostty", "zsh-aliases", "vim-keymaps", "scripts")

Output: a list of feature domains to process in subsequent steps.

## Step 2: Inventory Existing Skills

Search **both** skill locations to build a complete inventory:
...
2. **Choose name and location:**
   ...
   - Default: `.claude/skills/{name}/` — project-specific knowledge
   - Global only if genuinely cross-project: `~/.claude/skills/{name}/`
...
- **Both locations always** — check both `.claude/skills/` and `~/.claude/skills/`
