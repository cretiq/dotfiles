---
description: "Keep dotfiles CLAUDE.md accurate — verifies documented paths, conventions, and config against the real codebase. Prevents config drift."
allowed-tools: Bash(git diff:*), Bash(git log:*), Bash(git show:*)
---

# Update Dotfiles CLAUDE.md

Verify and update `CLAUDE.md` at the project root against the actual codebase.
...
|---|---|---|
| **Key Tools and Applications** | Config paths still valid, themes/fonts match | Read each config file, compare documented values |
| **Terminal and Shell** | Ghostty config, zsh config still accurate | Read `ghostty/.config/ghostty/config`, `zsh/.zshrc` |
| **Development Environment** | Vim/SPF config paths and settings | Read `vim/.vimrc`, `spf/.spf.toml` |
| **Applications** | App configs still exist at documented paths | Glob for config directories |
| **Git Configuration Management** | Bare repo alias still correct | Check `zsh/.zshrc` for config alias |
| **Vim Keymap Management** | Script paths, state files still valid | Check documented key files exist |
| **Port Management** | Aliases still in zshrc | Grep `zsh/.zshrc` for port aliases |
| **Application Shortcuts** | Aliases still defined | Grep `zsh/.zshrc` for `sp`, `mw`, `tm` |
| **Script Collection** | Scripts still exist at documented paths | Glob `my_scripts/.script/` |
| **Configuration Patterns** | Theme/font consistency still holds | Cross-check all config files |
| **File Structure** | Directory tree still matches | `ls` top-level dirs |

### For Each Section:

1. **Read the section** from CLAUDE.md
2. **Grep/Glob to verify** each documented path, pattern, or convention
3. **Check for new additions** — are there new configs, scripts, aliases not yet documented?
...
- **New application configs** not in Key Tools
- **New aliases or scripts** not documented
- **New keybindings** (Ghostty, BTT, etc.) not mentioned
- **Changed themes or fonts** across applications
...
- Code blocks for commands, paths, and config examples
...
### Verified Sections
- {section} — accurate / updated / stale content found
