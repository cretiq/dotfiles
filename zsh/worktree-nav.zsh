...
function c {
  if [[ "$1" == "mr" && -n "$2" ]]; then
    builtin cd "$PHOENIX_ROOT/p1" && claude --model "opus[1m]" --effort xhigh "/mr:review $2"
    return
  fi
  if [[ "$1" == "jira" && -n "$2" ]]; then
    local target=$(_wt_least_recent)
    builtin cd "$target" && claude --model "opus[1m]" --effort xhigh "/analyze-jira $2"
    return
  fi
  if [[ "$1" == @* ]]; then
    local input="${1#@}"
    ...
    builtin cd "$target" && claude --model "opus[1m]" --effort xhigh "${@:2}"
  else
...
function cc {
  if [[ "$1" == @* ]]; then
    ...
    local target=$(_wt_path "$wt" "$suffix")
    builtin cd "$target" && claude --continue --model "opus[1m]" --effort xhigh "${@:2}"
  else
    claude --continue --model "opus[1m]" --effort xhigh "$@"
  fi
}
